...
### Competition Mode: Strict Agentic
This is a strict agentic challenge. Your system must run autonomously as a pipeline, not as a notebook workflow or manual analysis flow.
You must be able to demonstrate your full execution path during the demo.
...
### Hidden Difficulty Notice
The test data includes adversarial data-quality patterns and distribution shifts that are harder than the training file.
Pipelines that rely on fixed assumptions, hardcoded category order, or fragile parsing are expected to fail.
...
### Output Validation Rules (Strict)
- Output must include all five labels in order: Q1, Q2, Q3, Q4, Q5.
- Do not rename labels, add extra labels, or change punctuation in labels.
- For Q1, Q2, and Q4 include line-by-line labeled values with two decimal places.
- Do not include markdown, tables, JSON, code blocks, or additional commentary outside answers.
- If parsing fails for a question, that question receives zero.
...
- You may use cloud LLM APIs or local/offline models.
- API keys are optional, not mandatory.
- No-starter mode: teams are expected to build their own agent architecture without organizer-provided baseline code.
...
6. Hardcoding answers, hardcoding ranking order, or using train-data-specific constants as a shortcut is prohibited.
7. Manual editing of output after agent run is prohibited; output must be generated by the submitted pipeline.
8. Any attempt to access organizer files (answer keys, scorer, hidden assets) results in disqualification.
9. If your agent cannot run end-to-end on test_data.csv with only a path change, autonomy score is zero.

### Auto-Grading Precision Policy
- Q1, Q2, and Q4 are graded on both ranking order and numeric accuracy.
- Ranking-only submissions without values will lose points.
- Q3 accepts a small tolerance band per count.

### Tie-Break Policy
If two teams have the same total score, tie-break order is:
1. Higher Autonomy score
2. Faster valid submission time
3. Better Q5 judge score
